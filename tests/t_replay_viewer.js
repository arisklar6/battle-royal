...
const gameVersion=Buffer.from("1");
const raw=Buffer.concat([
  Buffer.from(replay.MAGIC),u16(replay.VERSION),u16(24),
  Buffer.from([gameVersion.length]),gameVersion,u32(2),
  u32(0),u32(packets[0].length),packets[0],
  u32(24),u32(packets[1].length),packets[1],
]);
const v1Raw=Buffer.concat([
  Buffer.from(replay.MAGIC),u16(1),u16(24),u32(2),
  u32(0),u32(packets[0].length),packets[0],
  u32(24),u32(packets[1].length),packets[1],
]);
...
async function main(){
  ...
  assert.equal(decoded.gameVersion,"1");
